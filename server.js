...
async function getTopMovers() {
  try {
    // Primary: Alpaca most active / top gainers
...
Volume: ${snap.volMultiple}x average | Regime: ${marketRegime}
AI1 said YES technically. Is the catalyst strong enough to justify this ${dir} trade?
Consider catalyst type, magnitude, sustainability. Be objective — say NO if catalyst is weak or already priced in.
...
async function runAI5(sym, dir, catalyst, snap, a1, a2, a3, a4) {
  const prompt = `You are TITAN STOCK AI #5 — DEVIL'S ADVOCATE. Your entire purpose is maximum skepticism.
${sym} $${snap.price?.toFixed(2)} ${dir} | Catalyst: ${catalyst} | Volume: ${snap.volMultiple}x
AIs 1-4 all said YES. Your job: find every reason this trade will FAIL.
Ask yourself:
- Is this catalyst already priced in?
- Is this a pump & dump or manufactured news?
- What does smart money know that we don't?
- What's the bear case that the other AIs ignored?
- Has this exact setup historically failed?
Be the voice of caution. Only say YES if you genuinely cannot find a strong counter-argument.
Return ONLY JSON: {"verdict":"YES"|"NO","confidence":62-90,"reason":"devil's advocate assessment","primaryRisk":"biggest specific risk","counterArgument":"strongest bear case"}`;
  try {
...
async function run6AIPipeline(sym, dir, catalyst, catalystType, snap) {
  ...
  if (!a5 || a5.verdict !== 'YES') { console.log(`❌ ${sym} blocked by DEVIL'S ADVOCATE — ${a5?.primaryRisk}`); return null; }
...
async function runLiveScreener() {
  ...
  // Get real top movers from Alpaca
  const movers = await getTopMovers();

  // Build candidate list combining real movers + AI momentum scan
  let scanTargets = movers.slice(0, 15).map(m => ({ ticker: m.ticker, change: m.change, volume: m.volume, source: 'live_screener' }));

  // Filter — minimum move and volume
  scanTargets = scanTargets.filter(t => t.change >= 8 && t.volume > 50000);

  if (scanTargets.length === 0) {
    // Fallback: AI momentum scan when live data is thin
    await runAIMomentumScan();
    return;
  }

  // Get full snapshots for candidates
  const tickers = scanTargets.map(t => t.ticker);
  const snaps = await getMultiSnapshot(tickers);

  // Sort by volume multiple — highest conviction first
  const enriched = scanTargets
    .map(t => ({ ...t, snap: snaps[t.ticker] }))
    .filter(t => t.snap && t.snap.price > 0.50 && t.snap.price < 500)
    .sort((a, b) => (b.snap?.volMultiple || 0) - (a.snap?.volMultiple || 0));

  console.log(`📊 Live screener: ${enriched.length} candidates | SPY:${spyChange}% | Regime:${marketRegime}`);
  ...
  for (const candidate of enriched.slice(0, 5)) {
    ...
    if (positions[candidate.ticker]) continue; // already holding

    const sym = candidate.ticker;
    const snap = candidate.snap;

    // Determine direction — default LONG, SHORT if overextended
    const isOverextended = snap.change > 40 && snap.volMultiple < 3;
    const dir = isOverextended ? 'SHORT' : 'LONG';
    const catalystType = snap.change > 30 ? 'SQUEEZE' : snap.change > 15 ? 'MOMENTUM' : 'OTHER';
    const catalyst = `Live screener: ${sym} ${snap.change > 0 ? '+' : ''}${snap.change}% on ${snap.volMultiple}x volume | Regime: ${marketRegime}`;

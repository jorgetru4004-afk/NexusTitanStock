...
async function placeOrder(ticker, qty, side, extHours = true, limitPrice = null) {
  if (!ALPACA_KEY || qty <= 0) return null;
  try {
    // Extended hours requires limit orders not market orders
    const isExtHours = extHours && (limitPrice !== null);
    const body = {
      symbol: ticker, qty: Math.floor(qty), side,
      type: isExtHours ? 'limit' : 'market',
      time_in_force: 'day',
      extended_hours: extHours
    };
    if (isExtHours && limitPrice) body.limit_price = parseFloat(limitPrice.toFixed(2));
    const resp = await axios.post(`${ALPACA_BASE}/v2/orders`, body, { headers: alpacaHeaders(), timeout: 10000 });
    console.log(`📋 TITAN STOCK ${side} ${qty}x ${ticker} @ ${isExtHours ? '$'+limitPrice?.toFixed(2)+' LIMIT' : 'MARKET'} — ${resp.data?.id}`);
    return resp.data;
  } catch (e) {
...
async function enterPosition(sym, dir, catalyst, catalystType, snap, pipelineResult) {
  ...
  const order = await placeOrder(sym, qty, isShort ? 'sell' : 'buy', SETTINGS.extendedHours, entry);
